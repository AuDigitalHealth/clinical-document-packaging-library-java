...
public class PackageUtilityTest {
    ...
    public void testCreateZip() throws Exception {
        createZip(new SubmissionSet.Builder().rootDocument("rootdocument_filecontent".getBytes()).build());
    }
    ...
    public void testPlaceholder() throws Exception {
        ...
        writeZip(new SubmissionSet.Builder()
    ...
    public void testWriteZip() throws Exception {
        SubmissionSet submissionSet = extractPackage("src/test/resources/BasePackage.zip");
        // PackagingUtility.writeZip(submissionSet, "src/test/resources/NewPackage.zip");
        SubmissionSet newPackage = extractPackage("src/test/resources/NewPackage.zip");
        Assert.assertEquals(submissionSet, newPackage);
    }

    @Test
    public void testVerifySignature() throws Exception {
        SubmissionSet pkg = extractPackage("src/test/resources/BasePackage.zip");
        verifyPackage(pkg, null);
    }

    @Test(expected = VerificationException.class)
    public void testVerifySignatureModifiedSignature() throws Exception {
        SubmissionSet pkg = extractPackage("src/test/resources/BasePackageBadSig.zip");
    ...
    public void testVerifySignatureModifiedRoot() throws Exception {
        SubmissionSet pkg = extractPackage("src/test/resources/BasePackageBadRoot.zip");
        try {
            verifyPackage(pkg, null);
        } catch (SignatureGenerationException e) {
    ...
    public void testExtractingALargePackage() throws Exception {
        SubmissionSet pkg = extractPackage("src/test/resources/BigPackage.zip");
        try {
            verifyPackage(pkg, null);
        } catch (SignatureGenerationException e) {
            Assert.assertTrue(e.getMessage().contains("Root hash in the eSignature does not match the package's root document:"));
    ...
    public void testExtractingVicHealthPackage() throws Exception {
        ...
        SubmissionSet pkg = extractPackage("src/test/resources/1.3.16.1.38818.231374586-vic-health.zip");
    ...
    public void testNonStandardNames() throws Exception {
        SubmissionSet pkg = extractPackage("src/test/resources/CDA.zip");
        verifyPackage(pkg, null);
    }
     ...
     * @return The created approver's name
     */
